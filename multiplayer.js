...
export async function pushAction(code, action){
  // action = { type: 'spin'|'guess'|'solve'|'buyVowel', payload: {...} }
  await update(stateRef(code), {
    lastAction: { ...action, ts: Date.now() }
  });
}
...
window.MPapi = { createRoom, joinRoom, listenRoom, stopListening,
                  pushGameState, pushScore, pushTurn, pushAction, closeRoom };
