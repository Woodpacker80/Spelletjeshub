...
export async function submitScore(difficulty, score){
  ...
  // Zoek bestaande entry van deze speler (zelfde naam + token)
  ...
  // Verwijder alleen de eigen entry van dit apparaat (niet anderen met zelfde naam)
  if(existingKey){
    await remove(ref(db, `${path}/${existingKey}`));
  }

  // Sla nieuwe high score op
...
// ── Fetch leaderboard — top 5 unieke spelers ─────────────────────────────────
export async function fetchLeaderboard(difficulty){
  const path = `${GAME_KEY}/${difficulty}`;
  // Haal meer op dan TOP_N zodat we duplicaten kunnen filteren
  ...
  // Bewaar alleen de hoogste score per unieke naam
  const seen = new Set();
  const top  = [];
  for(const e of all){
    if(!seen.has(e.name)){
