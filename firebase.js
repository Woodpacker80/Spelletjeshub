// ═══════════════════════════════════════════════════════
//  FIREBASE LEADERBOARD — rad-van-woorden v13
// ═══════════════════════════════════════════════════════
...
// ── Haal alle entries op zonder query of filter ──────────────────────────────
async function fetchAll(difficulty){
  ...
  snap.forEach(child => {
    const val = child.val();
    // Zorg dat we alleen geldige entries toevoegen
    if(val && typeof val.score === 'number' && val.name){
      entries.push(val);
    }
  });
  // Debug: toon aantal entries in de console
  console.log(`fetchAll(${difficulty}): ${entries.length} entries gevonden`, entries);
  return entries;
}

// ── Check of score in top 10 past ────────────────────────────────────────────
