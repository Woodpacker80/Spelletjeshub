...
function drawWheel(rot){
  ...
  for(let i=0;i<n;i++){
    ...
    if(seg.val==='BONUS_CHROME'){
      const g=ctx.createLinearGradient(cx-r,cy,cx+r,cy);
      g.addColorStop(0,'#8a9ba8'); g.addColorStop(0.25,'#c8d6e0');
      g.addColorStop(0.5,'#f0f4f7'); g.addColorStop(0.75,'#c8d6e0');
      g.addColorStop(1,'#8a9ba8'); ctx.fillStyle=g;
    } else if(seg.val==='BONUS_GOLD'){
      const g=ctx.createLinearGradient(cx-r,cy,cx+r,cy);
      g.addColorStop(0,'#7a4800'); g.addColorStop(0.25,'#c97d00');
      g.addColorStop(0.5,'#f5c518'); g.addColorStop(0.75,'#c97d00');
      g.addColorStop(1,'#7a4800'); ctx.fillStyle=g;
    ...
    if(seg.val==='BONUS_CHROME'){
      ...
      ctx.fillText('BONUS',r-5,-fs*0.55);
      ctx.font=`bold ${Math.max(5,Math.round(fs*0.7))}px Oswald,sans-serif`;
      ctx.fillText('+€2500',r-5,fs*0.55);
    } else if(seg.val==='BONUS_GOLD'){
      ctx.shadowColor='rgba(120,70,0,0.7)'; ctx.shadowBlur=4;
      ctx.fillStyle='#3a1a00';
      ctx.font=`bold ${fs}px Oswald,sans-serif`;
      ctx.fillText('BONUS',r-5,-fs*0.55);
      ctx.font=`bold ${Math.max(5,Math.round(fs*0.7))}px Oswald,sans-serif`;
      ctx.fillText('+€5000',r-5,fs*0.55);
    } else if(seg.val==='MINUS'){
      ctx.shadowColor='rgba(0,0,0,0.5)'; ctx.shadowBlur=2;
      ctx.fillStyle='#000';
      ctx.font=`bold ${fs}px Oswald,sans-serif`;
      ctx.fillText('MINUS',r-5,-fs*0.55);
      ctx.font=`bold ${Math.max(5,Math.round(fs*0.7))}px Oswald,sans-serif`;
      ctx.fillText('-50%',r-5,fs*0.55);
    } else {
      ctx.fillStyle='#fff'; ctx.shadowColor='rgba(0,0,0,0.9)'; ctx.shadowBlur=3;
      ctx.font=`bold ${fs}px Oswald,sans-serif`;
      ctx.fillText(seg.lbl,r-5,fs*0.37);
    }
    ctx.restore();
  }
...
function startRound(){
  ...
  if(roundType === 'chrome'){
    const base = [...SEGS];
    const moneyIdxs = base.map((seg,i)=>typeof seg.val==='number'?i:-1).filter(i=>i>=0);
    const shuffled  = [...moneyIdxs].sort(()=>Math.random()-0.5);
    const positions = [shuffled[0], shuffled[1]].sort((a,b)=>b-a);
    base.splice(positions[0], 0, BONUS_SEGS.chrome[0]);
    base.splice(positions[1], 0, BONUS_SEGS.chrome[1]);
    activeSegs = base;

  } else if(roundType === 'gold'){
    const base = [...SEGS];

    // Stap 1: Goudvak direct na FAILLIET
    const bkIdx = base.findIndex(seg => seg.val === 'BANKRUPT');
    base.splice(bkIdx + 1, 0, BONUS_SEGS.gold[0]);

    // Stap 2: Chroomvak op een willekeurige geldpositie
    const moneyIdxs1 = base.map((seg,i)=>typeof seg.val==='number'?i:-1).filter(i=>i>=0);
    const chromePos  = moneyIdxs1[Math.floor(Math.random()*moneyIdxs1.length)];
    base.splice(chromePos, 0, BONUS_SEGS.chrome[0]);

    // Stap 3: Wissel precies ÉÉN Beurt Verlies naar Min
    const loseIdx = base.findIndex(seg => seg.val === 'LOSE');
    if(loseIdx >= 0) base[loseIdx] = { lbl:'MINUS', val:'MINUS', col:'#c0392b' };

    // Stap 4: Vervang één willekeurig geldvak door tweede Min
    const moneyIdxs2 = base.map((seg,i)=>typeof seg.val==='number'?i:-1).filter(i=>i>=0);
    const minusPos   = moneyIdxs2[Math.floor(Math.random()*moneyIdxs2.length)];
    base[minusPos]   = { lbl:'MINUS', val:'MINUS', col:'#c0392b' };
  ...
  if(roundType === 'gold'){
    ...
    pill.textContent = '⭐ Chroomronde ' + G.round + ' van ' + G.maxRounds;
  ...
  if(roundType !== 'normal'){
    ...
    if(roundType === 'gold'){
      title = '👑 Goudronde!';
      body  = 'Goudvak (+€5.000) staat naast Failliet. Chroomvak (+€2.500) staat ook op het rad. Één Beurt Verlies is nu een Min-vak (−50%). Klinkers kosten €500 in deze ronde!';
      icon  = '✨';
    } else {
      title = '⭐ Chroomronde!';
      body  = 'Chroomvak: land erop + raad een medeklinker goed = verdiensten + €2.500! Min-vak: verlies 50% rondeverdiensten!';
      icon  = '⚡';
    }
    setTimeout(()=>{
...
function onLanded(seg,who){
  if(seg.val==='BANKRUPT'){
    ...
    setBanner('🥈 CHROOM BONUS — raad een letter!','bonus');
...
function computerTurn(){
  ...
  setTimeout(()=>{
    ...
    if(G.roundScore.computer>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&pct>vowelMinPct&&Math.random()<vowelChance){
