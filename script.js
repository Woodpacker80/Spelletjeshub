...
function handlePlayButton(){
  ...
  const panel = document.getElementById('debug-panel');
  if(panel){ panel.style.display='block'; panel.innerHTML+=`<div style="color:#ff0">[handlePlayButton] FB=${!!window.FB} nickname=${window.FB?window.FB.getNickname():'n/a'} val="${val}"</div>`; }

  // If FB available and nickname already stored, use it silently
  if(window.FB && window.FB.getNickname()){
    if(panel) panel.innerHTML+=`<div style="color:#ff0">Nickname found — showing pinball</div>`;
    showPinball(G.difficulty || 'medium', ()=> startGame()); return;
  }
  // Validate name input
  if(val.length < 3){
  ...
  if(panel) panel.innerHTML+=`<div style="color:#ff0">Name saved — showing pinball</div>`;
...
async function showPinball(difficulty, onDone){
  ...
  // After 3.5s fade out and call onDone
  setTimeout(()=>{
