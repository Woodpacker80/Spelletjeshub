...
function updateTurnUI(){
  ...
  document.getElementById('btn-spin').disabled=!(isP&&(G.phase==='spin'||G.phase==='guess')&&!G.locked&&!G.suddenDeath);
  document.getElementById('btn-vowel').disabled=!(isP&&G.phase==='guess'&&!G.locked&&G.roundScore.player>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&!G.suddenDeath);
...
// ── DRAAIEN ───────────────────────────────────────────────────────────────────
function handleSpin(){
  if(G.spinning||G.turn!=='player'||(G.phase!=='spin'&&G.phase!=='guess')||G.locked) return;
  doSpin('player');
}
...
function handleJoker(who){
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      G.locked = false; G.phase = 'guess'; G.spinVal = 0; G.onBonus = false;
      updateTurnUI();
      if(who !== 'player') setTimeout(()=>computerTurn(), 1000);
    }, delay);
  }, 1200);
...
function handleBuyVowel(){
  if(G.turn!=='player'||G.phase!=='guess'||G.locked) return;
