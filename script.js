...
const DIFF = {
  easy: {
    ...
    solve:      0.80,   // % revealed before attempting to solve (jouw drempel)
    solveChance:0.40,
    failSolve:  0.10,   // kans op Illusionist-fail bij reguliere oplospoging
    thinkDelay: 1400,
  },
  medium: {
    freq:       FREQ_ORDER,
...
function computerTurn(){
  ...
  setTimeout(()=>{
    ...
    if(noConsonantsLeft){
      ...
      if(pct < cfg.solve){
        setBanner('🤖 Foute oplospoging…','bad');
        setTimeout(()=>{
          sndWrong();
          setBanner('❌ Computer raadt verkeerd!','bad');
          setTimeout(()=>switchTurn('computer'), 1000);
        }, 800); return;
      }
    ...
    if(pct >= cfg.solve && Math.random() < cfg.solveChance){
      ...
      if(Math.random() < failChance){
        // Illusionist: doe alsof je oplost maar fail bewust
        setBanner('🤖 Foute oplospoging…','bad');
        setTimeout(()=>{
          sndWrong();
          setBanner('❌ Computer raadt verkeerd!','bad');
...
async function showWinLeaderboard(difficulty, myScore){
  return new Promise(async (resolve)=>{
    try {
      ...
      diffLabel.textContent = (DIFF_LABELS[difficulty] || difficulty.toUpperCase()) + ` (${entries.length} entries)`;
