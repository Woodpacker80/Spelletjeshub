...
// ── RONDE ─────────────────────────────────────────────────────────────────────
function pickPuzzle(){
  // Filter out puzzles where any single word exceeds 21 characters
  const MAX_WORD_LEN = 21;
  const eligible = PUZZLES.filter(p =>
    p.a.split(' ').every(w => w.length <= MAX_WORD_LEN)
  );
  const pool = eligible.length > 0 ? eligible : PUZZLES;
  // Track used indices within the filtered pool
...
function renderPuzzle(){
  ...
  // We try tile widths from 26 down to 14 until the longest word fits.
