...
function doSpin(who, forcedRot){
  G.spinning=true; G.locked=true; updateTurnUI();

  let totalRot;
  if(forcedRot !== undefined){
    // Tegenstander draait: gebruik de rotatie die via Firebase binnenkwam
    totalRot = forcedRot;
  } else {
    // Wij draaien zelf: genereer rotatie en stuur DIRECT naar tegenstander
    totalRot = (7+Math.random()*8)*Math.PI*2;
    if(G.mpMode && !G.mpSuppressActionPush && mpIsMyTurn()){
      mpPushAction({ type:'spin', totalRot: totalRot });
    }
  }

  // Koppel draaitijd aan rotatie zodat beide schermen exact tegelijk klaar zijn
...
function mpProcessOppAction(a){
  ...
  switch(a.type){
    ...
    case 'spin':
      // Tegenstander heeft gedraaid — exact dezelfde animatie uitvoeren
      G.locked = true;
      doSpin(oppTurn, a.totalRot);
      break;
