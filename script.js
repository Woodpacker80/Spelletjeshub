...
function endRound(winner){
  ...
  setTimeout(()=>showResult(title,sub,icon,true,()=>{
    ...
    if(G.mpMode && window.MP.role==='host'){
      startRound();           // laadt nieuwe puzzel
      mpStartCoinFlip();      // bepaalt wie begint
    } else if(G.mpMode){
      startRound();           // gast laadt ook puzzel
      // gast wacht op coinflip van host via Firebase
      G.locked = true;
      setBanner('Wachten op muntgooien...', 'empty');
    } else {
      startRound();
    }
  }),500);
...
function mpStartGame(code, room){
  ...
  if(window.MP.role === 'host'){
    setTimeout(() => mpStartCoinFlip(), 800);
  } else {
...
// ═══════════════════════════════════════════════════════════════════════════
//  MUNTGOOIEN — bepaalt wie elke ronde begint
//  Alleen de host gooit de munt en pusht het resultaat naar Firebase.
//  Beide spelers zien een korte animatie en het spel start direct.
// ═══════════════════════════════════════════════════════════════════════════

function mpStartCoinFlip(){
  G.locked = true;

  if(window.MP.role === 'host'){
    // Host bepaalt willekeurig wie begint
    const starter = Math.random() < 0.5 ? 'player1' : 'player2';
    // Push direct naar Firebase zodat gast het ook ziet
    ...
    mpShowCoinFlipResult(starter);
  }
  // Gast wacht op coinflip actie via mpHandleAction
}

function mpShowCoinFlipResult(starter){
  const starterNick = starter === 'player1' ? mpGetP1Nick() : mpGetP2Nick();
  const myTurn = starter === window.MP.myKey;

  // Toon overlay bericht — kort en krachtig
  setBanner('🪙 ' + starterNick + ' begint!', 'money');

  // Na 1.5 seconden direct de ronde starten
  setTimeout(() => {
    setBanner('', 'empty');
...
function mpHandleAction(a){
  ...
  switch(a.type){
    ...
    case 'coinflip':
      // Gast ontvangt het muntgooi resultaat van de host
      if(window.MP.role === 'guest'){
        setTimeout(() => mpShowCoinFlipResult(a.starter), 300);
      }
      break;
