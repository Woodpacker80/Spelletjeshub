...
async function endGame(){
  ...
  if(outcome === 'win'){
    setTimeout(async ()=>{
      await trySubmitScore(myTotal, G.difficulty);
...
function stopPinballHum(){
  _pinballHumNodes.forEach(({osc,gain})=>{
    try { gain.gain.setTargetAtTime(0, AC.currentTime, 0.2); osc.stop(AC.currentTime+0.3); } catch(e){}
  });
  _pinballHumNodes = [];
}
...
function mpStartGame(code, room){
  ...
  const myNick  = window.MP.myKey === 'player1' ? p1nick : p2nick;
  const oppNick = window.MP.myKey === 'player1' ? p2nick : p1nick;
...
async function mpCleanup(){
  ...
  if(mpUnlisten){ mpUnlisten(); mpUnlisten = null; }
  // Reset namen terug naar single player labels
