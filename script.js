...
function updateTurnUI(){
  // In MP: isP = is het mijn beurt? (gast is 'computer' in de staat)
...
function mpBeginRound(starter){
  ...
  // host = player1 = 'player', gast = player2 = 'computer'
  // starter is 'player1' of 'player2'
  ...
  if(window.MP.role === 'host'){
    // Host start de ronde en pusht de staat
    startRound();
    // startRound reset G.turn — zet het opnieuw correct
    G.turn   = (starter === 'player1') ? 'player' : 'computer';
    G.locked = !mpIsMyTurn();
    mpPushState();
  }
...
function mpUpdateScorebar(room){
  ...
  // p1 = host = 'player', p2 = gast = 'computer'
  document.getElementById('mp-p1').classList.toggle('active-mp', G.turn === 'player');
  document.getElementById('mp-p2').classList.toggle('active-mp', G.turn === 'computer');
}
...
// Is het mijn beurt?
// 'player' = host (player1), 'computer' = gast (player2) — altijd
function mpIsMyTurn(){
  if(!window.MP) return false;
  if(window.MP.myKey === 'player1') return G.turn === 'player';
  if(window.MP.myKey === 'player2') return G.turn === 'computer';
  return false;
}
