...
function switchTurn(){
  ...
  G.phase='spin'; G.spinVal=0; G.locked=false; G.hasGuessed=false;
...
// ═══════════════════════════════════════════════════════════════════════════
//  MULTIPLAYER UI — Rad van Woorden
// ═══════════════════════════════════════════════════════════════════════════
...
async function mpJoinRoom(){
  ...
  try {
    ...
    closeMpLobby();
    mpStartGame(code, { ...room, status:'playing',
...
async function mpCancelWaiting(){
  ...
  if(window.MP && window.MP.roomCode){
    await window.MPapi.closeRoom(window.MP.roomCode);
  }
  document.getElementById('modal-mp-waiting').classList.remove('open');
}

// ── Spel starten in multiplayer modus ────────────────────────────────────────
function mpStartGame(code, room){
  // Stop eventuele oude listener
  if(mpUnlisten){ mpUnlisten(); mpUnlisten = null; }

  // Zet namen in scorebar
  ...
  G.mpMode   = true;
  G.mpCode   = code;
  G.mpMyKey  = window.MP.myKey;   // 'player1' of 'player2'
  G.mpOppKey = window.MP.oppKey;
  G.turn     = window.MP.role === 'host' ? 'player' : 'computer';
  ...
  // Host stuurt eerste spelstaat, gast luistert
  if(window.MP.role === 'host'){
    startRound();
    mpPushState(); // stuur beginstaat naar Firebase
  } else {
    setBanner('Wachten op tegenstander...', 'empty');
  }

  // Beide luisteren naar updates
  mpUnlisten = window.MPapi.listenRoom(code, mpOnRoomUpdate);
}

// ── Spelstaat naar Firebase sturen (host doet dit na elke actie) ─────────────
function mpPushState(){
  if(!G.mpMode || !G.mpCode) return;
  if(window.MP.role !== 'host') return; // alleen host beheert de staat
...
function mpOnRoomUpdate(room){
  ...
  if(window.MP.role === 'host') return; // host beheert zelf, luistert niet naar state

  const st = room.state;
  if(!st.ts || st.ts <= mpLastActionTs) return; // verouderde update
  mpLastActionTs = st.ts;

  // Herstel spelstaat van host
  G.round       = st.round;
  G.turn        = st.turn;
  G.phase       = st.phase;
  G.puzzle      = st.puzzle;
