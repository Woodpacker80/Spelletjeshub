...
function drawWheel(rot){
  ...
  for(let i=0;i<n;i++){
    ...
    if(seg.val==='BONUS_GOLD'){
      ctx.shadowColor='rgba(80,40,0,0.9)'; ctx.shadowBlur=4;
      ctx.fillStyle='#2a1000';
      ctx.font='bold '+fs+'px Oswald,sans-serif';
      ctx.fillText('+€5000',r-6,fs*0.37);

    } else if(seg.val==='BONUS_CHROME'){
      ctx.shadowColor='rgba(0,20,40,0.8)'; ctx.shadowBlur=4;
      ctx.fillStyle='#0a1520';
      ctx.font='bold '+fs+'px Oswald,sans-serif';
      ctx.fillText('+€2500',r-6,fs*0.37);
...
function startRound(){
  ...
  if(roundType === 'chrome'){
    ...
    // Minimum 4 segmenten afstand tussen de twee Min-vakjes (circulaire afstand)
    const minus1Idx = base.findIndex(seg => seg.val === 'MINUS');
    const moneyIdxs2 = base.map((seg,i)=>typeof seg.val==='number'?i:-1).filter(i=>i>=0);
    const totalSegs  = base.length;
    const MIN_GAP    = 4;
    const shuffled2  = [...moneyIdxs2].sort(()=>Math.random()-0.5);
    let minusPos = shuffled2[0];
    for(const pos of shuffled2){
      const dist = Math.min(
        Math.abs(pos - minus1Idx),
        totalSegs - Math.abs(pos - minus1Idx)
      );
      if(dist >= MIN_GAP){ minusPos = pos; break; }
    }
