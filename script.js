...
function doSpin(who){
  ...
  function frame(now){
    ...
    const arc=(2*Math.PI)/_segs.length;
    // Segments drawn at rot+i*arc-PI/2; pointer at top (-PI/2)
    // Correct formula for counterclockwise: use -fRot
    const pAngle=(((-fRot)%(2*Math.PI))+2*Math.PI)%(2*Math.PI);
    const fsi=Math.floor(pAngle/arc)%_segs.length;
