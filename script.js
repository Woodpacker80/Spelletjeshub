...
let _currentLbTab = 'easy';

// Called at game start — check if nickname exists
function checkNicknameAndStart(){
  if(window.FB && window.FB.getNickname()){
    startGame();
  } else {
    showScreen('screen-nickname');
    setTimeout(()=>{ const el=document.getElementById('nickname-input'); if(el) el.focus(); },300);
  }
}
...
function saveNickname(){
  ...
  if(val.length < 3){
    error.textContent = 'Minimaal 3 tekens vereist';
    return;
  }
  if(val.length > 12){
    error.textContent = 'Maximaal 12 tekens toegestaan';
    return;
  }
...
async function showLeaderboard(){
  ...
  switchLbTab(_currentLbTab);
}

async function switchLbTab(diff){
  _currentLbTab = diff;
  // Update tab buttons
  document.querySelectorAll('#lb-diff-sel .opt-btn').forEach(b=>{
    b.classList.toggle('sel', b.dataset.ld === diff);
  });
  // Show loading
  ...
  if(!window.FB){
    list.innerHTML = '<div class="lb-empty">Scorebord niet beschikbaar</div>';
    return;
  }
  ...
  try {
    ...
    if(!entries.length){
      list.innerHTML = '<div class="lb-empty">Nog geen scores.<br>Wees de eerste! 🏆</div>';
      return;
    }
    const rankIcons = ['🥇','🥈','🥉'];
    ...
    list.innerHTML = '<div class="lb-empty">Kon scorebord niet laden.<br>Controleer je verbinding.</div>';
  }
}
