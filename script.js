...
function switchTurn(requester = null){
  ...
  if(G.mpMode){
    ...
    updateTurnUI();
    if(!G.mpSuppressActionPush) mpPushAction({ type:'switch_turn', turn: G.turn });
  } else {
    G.locked = false;
...
async function mpCreateRoom(){
  ...
  if(!window.MP || !window.MP.myKey === undefined){ mpStatus('Multiplayer nog niet gereed, probeer opnieuw.'); return; }
...
async function mpJoinRoom(){
  if(!window.MPapi){ mpStatus('Multiplayer niet beschikbaar.'); return; }
  if(!window.MP || !window.MP.myKey){ mpStatus('Multiplayer nog niet gereed, probeer opnieuw.'); return; }
