...
function updateTurnUI(){
  ...
  document.getElementById('btn-spin').disabled=!(isP&&(G.phase==='spin'||G.phase==='guess'||G.phase==='post_guess')&&!G.locked&&!G.suddenDeath);
  document.getElementById('btn-vowel').disabled=!(isP&&(G.phase==='guess'||G.phase==='post_guess')&&!G.locked&&G.roundScore.player>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&!G.suddenDeath);
...
// ── DRAAIEN ───────────────────────────────────────────────────────────────────
function handleSpin(){
  if(G.spinning||G.turn!=='player'||(G.phase!=='spin'&&G.phase!=='guess'&&G.phase!=='post_guess')||G.locked) return;
  doSpin('player');
}
...
function handleJoker(who){
  ...
  setTimeout(()=>{
    ...
    const earn = 1000 * count;
    G.roundScore[who] += earn;
    updateScores();
    const earnStr = earn.toLocaleString('nl-NL');
    setBanner(`Joker ⭐ ${letter} zit er ${count}x in! +€${earnStr}`,'bonus');

    const delay = count > 0 ? (count * 400 + 500) : 500;
    setTimeout(()=>{
      if(isPuzzleSolved()){ endRound(who); return; }
      G.locked = false; G.phase = 'post_guess'; G.spinVal = 0; G.onBonus = false;
      updateTurnUI();
      if(who !== 'player') setTimeout(()=>computerTurn(), 1000);
    }, delay);
  }, 1200);
...
function handleKeyPress(letter){
  ...
  if(G.phase==='post_guess') return; // must spin or buy vowel first
...
function handleBuyVowel(){
  if(G.turn!=='player'||(G.phase!=='guess'&&G.phase!=='post_guess')||G.locked) return;
  ...
  setTimeout(()=>{
    ...
    if(hit){
      ...
      if(G.onBonus && G.bonusVal){
        ...
        const label = G.roundType==='gold' ? 'Goud Bonus 👑' : 'Zilver Bonus 🥈';
      ...
      G.phase='post_guess'; G.locked=false; updateTurnUI();
...
function computerTurn(){
  ...
  setTimeout(()=>{
    showThinking(false);
    // Near-solve: only vowels missing — buy one or solve
    if(onlyVowelsLeft){
      const av=[...VOWELS].filter(v=>!G.usedLetters.has(v)&&G.puzzle.a.includes(v));
      if(av.length>0){
        if(G.roundScore.computer>=(G.vowelCost||VOWEL_COST)){
