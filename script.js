...
async function endGame(){
  ...
  if(outcome === 'win'){
    ...
    if(panel){ panel.style.display='block'; panel.innerHTML+=`<div style="color:#ff0">[endGame] WIN — score=${G.totalScore.player} diff=${G.difficulty}</div>`; }
    setTimeout(async ()=>{
...
function handlePlayButton(){
  ...
  const panel = document.getElementById('debug-panel');
  if(panel){ panel.style.display='block'; panel.innerHTML+=`<div style="color:#ff0">[handlePlayButton] FB=${!!window.FB} FBready=${!!(window.FB&&window.FB.getNickname!==undefined)} val="${val}"</div>`; }
  ...
  if(!storedName){
    ...
    if(panel) panel.innerHTML+=`<div style="color:#ff0">Stored name: ${storedName}</div>`;
  }

  if(panel) panel.innerHTML+=`<div style="color:#ff0">Calling showPinball...</div>`;
  showPinball(G.difficulty || 'medium', ()=> startGame());
}
...
async function showPinball(difficulty, onDone){
  ...
  const entriesEl= document.getElementById('pinball-entries');
  const diffLabel= document.getElementById('pinball-diff-label');
  const panel    = document.getElementById('debug-panel');
  if(panel) panel.innerHTML+=`<div style="color:#0ff">[showPinball] overlay=${!!overlay} diff=${difficulty}</div>`;
  ...
  entriesEl.innerHTML    = '<div class="pinball-entry show rank-other" style="justify-content:center;color:var(--muted);letter-spacing:2px;font-size:13px">LADEN...</div>';
  ...
  overlay.style.opacity    = '1';
  if(panel) panel.innerHTML+=`<div style="color:#0ff">Overlay shown ✓</div>`;

  // Start 3.5s countdown immediately — independent of fetch
  const hideOverlay = ()=>{
    overlay.style.opacity='0';
    setTimeout(()=>{
      overlay.classList.remove('visible');
  ...
  // Play retro synth hum
  playPinballHum();

  // Fetch entries (non-blocking — will update while visible)
  let entries = [];
  if(window.FB){
    try { entries = await window.FB.fetchLeaderboard(difficulty); } catch(e){
      if(panel) panel.innerHTML+=`<div style="color:#f00">fetchLeaderboard error: ${e.message}</div>`;
    }
  }
  entries = entries.slice(0,5);
  if(panel) panel.innerHTML+=`<div style="color:#0ff">Entries: ${entries.length}</div>`;
  ...
  if(!entries.length){
    entriesEl.innerHTML = '<div class="pinball-entry show rank-other" style="justify-content:center;color:var(--muted);letter-spacing:2px;font-size:13px">NOG GEEN SCORES</div>';
  } else {
    entriesEl.innerHTML = '';
    entries.forEach((e,i)=>{
      const isMe = myName && e.name === myName;
      const div  = document.createElement('div');
      div.className = `pinball-entry ${rankClass[i]}${isMe?' is-you':''}`;
      div.innerHTML = `<span class="pe-rank">${rankIcons[i]}</span>
        <span class="pe-name">${e.name}${isMe?' ◀':''}</span>
        <span class="pe-score">€${e.score.toLocaleString('nl-NL')}</span>`;
      entriesEl.appendChild(div);
      setTimeout(()=> div.classList.add('show'), 100 + i * 180);
    });
  }
}
...
async function trySubmitScore(score, difficulty){
  if(!window.FB || !window.FB.submitScore){
    const panel = document.getElementById('debug-panel');
    if(panel){ panel.style.display='block'; panel.innerHTML+='<div style="color:#f00">window.FB not available!</div>'; }
    return;
  }
  try {
    ...
    const panel = document.getElementById('debug-panel');
    if(panel){ panel.style.display='block'; panel.innerHTML+=`<div style="color:#f00">submitScore error: ${e.message}</div>`; }
  }
}
...
// ── Menu leaderboard (full modal, all 15) ────────────────────────────────────
function menuLeaderboard(){
  document.getElementById('menu-btn').classList.remove('open');
  document.getElementById('menu-dropdown').classList.remove('open');
  document.getElementById('modal-leaderboard').classList.add('open');
  switchModalLbTab(_modalLbTab);
}
function closeLeaderboardModal(){
  document.getElementById('modal-leaderboard').classList.remove('open');
}

async function switchModalLbTab(diff){
  _modalLbTab = diff;
  document.querySelectorAll('#modal-lb-diff-sel .opt-btn').forEach(b=>{
    b.classList.toggle('sel', b.dataset.mld === diff);
  });
  const list = document.getElementById('modal-lb-list');
  list.innerHTML = '<div class="lb-loading">Laden...</div>';
  if(!window.FB){ list.innerHTML='<div class="lb-empty">Niet beschikbaar</div>'; return; }
  try {
    const entries = await window.FB.fetchLeaderboard(diff);
    const myName  = window.FB.getNickname();
    if(!entries.length){
      list.innerHTML = '<div class="lb-empty">Nog geen scores op dit niveau</div>'; return;
    }
    const rankIcons = ['🥇','🥈','🥉'];
    const rankClass = ['gold','silver','bronze'];
    list.innerHTML = entries.map((e,i)=>{
    ...
    list.innerHTML = '<div class="lb-empty">Kon scorebord niet laden</div>';
  }
}
