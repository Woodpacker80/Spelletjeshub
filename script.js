...
function startRound(){
  ...
  if(roundType !== 'normal'){
    ...
    if(roundType === 'gold'){
      ...
      body  = 'Een gouden BONUS-vak (+€5.000) staat naast Bankrupt en een zilveren BONUS-vak (+€2.500) staat ook op het rad. Er zijn ook twee <span class="star-blue">★</span> JOKER-vakken en één Min-vak in het spel. Land op de JOKER om een willekeurige letter te onthullen! Klinkers kosten €500 in deze ronde!';
      icon  = '👑';
    } else {
...
function updateTurnUI(){
  ...
  document.getElementById('btn-vowel').disabled=!(isP&&(G.phase==='guess'||(G.phase==='spin'&&G.hasGuessed))&&!G.locked&&G.roundScore.player>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&!G.suddenDeath);
...
function switchTurn(){
  ...
  if(G.mpMode){
    // In MP: computer = tegenstander op andere telefoon
    // Host stuurt nieuwe staat, beide kanten updaten UI
    const myTurn = (G.turn === 'player');
    G.locked = !myTurn;
...
function onLanded(seg,who){
  // Spin-off intercept — als we in spin-off modus zijn, verwerk het resultaat apart
  if(G.mpSpinOffPending){
    mpHandleSpinOffResult(seg);
    return;
  }
  if(seg.val==='BANKRUPT'){
...
function handleBuyVowel(){
  if(G.turn!=='player'||(G.phase!=='guess'&&(G.phase!=='spin'||!G.hasGuessed))||G.locked) return;
  ...
  setTimeout(()=>{
    ...
    if(hit){
      ...
      if(G.onBonus && G.bonusVal){
        ...
        const label = G.bonusType==='gold' ? 'Goud Bonus 👑' : 'Zilver Bonus 🥈';
...
function endRound(winner){
  ...
  if(G.mpMode){ mpPushState(); window.MPapi.pushScore(G.mpCode, G.mpMyKey, G.totalScore.player); }
...
async function endGame(){
  ...
  if(outcome === 'win'){
    setTimeout(async ()=>{
      await trySubmitScore(G.totalScore.player, G.difficulty);
      // After leaderboard fades out — fade win screen and go to credits
    ...
    // Loss or tie — straight to credits after 5s
    setTimeout(()=>{
      overlay.style.opacity='0';
      setTimeout(()=>{
        overlay.style.pointerEvents='none';
...
function stopPinballHum(){
  _pinballHumNodes.forEach(({osc,gain})=>{
    try { gain.gain.setTargetAtTime(0, audioCtx.currentTime, 0.2); osc.stop(audioCtx.currentTime+0.3); } catch(e){}
  });
  _pinballHumNodes = [];
}
...
async function trySubmitScore(score, difficulty){
  if(!window.FB || !window.FB.submitScore){
    return;
  }
  try {
  ...
  return new Promise(async (resolve)=>{
    ...
    list.innerHTML = entries.map((e,i)=>{
      ...
      return `<div class="lb-entry${isMe?' lb-you lb-new-entry':''}">
        ${rank}
        <span class="lb-name">${e.name}${isMe?' ⟵ jij':''}</span>
        <span class="lb-score">€${e.score.toLocaleString('nl-NL')}</span>
      </div>`;
    }).join('');

    // Auto-hide after 4 seconds then resolve
    setTimeout(()=>{
      overlay.style.transition='opacity 0.6s ease';
      overlay.style.opacity='0';
      setTimeout(()=>{
...
// ═══════════════════════════════════════════════════════════════════════════
//  MULTIPLAYER UI — Rad van Woorden  v2
//  Architectuur: host = spelbaas, gast ontvangt acties via Firebase
//  Firebase structuur:
//    /multiplayer/{code}/action  — laatste actie van actieve speler
//    /multiplayer/{code}/state   — volledige spelstaat (door host gepusht)
//    /multiplayer/{code}/presence/{key} — online status per speler
// ═══════════════════════════════════════════════════════════════════════════
...
function spinOffValue(seg){
  if(SPINOFF_VAL[seg.val] !== undefined) return SPINOFF_VAL[seg.val];
  return typeof seg.val === 'number' ? seg.val : 0;
}
...
async function mpJoinRoom(){
  ...
  try {
    ...
    mpStartGame(code, {
      ...room, status:'playing',
      player2:{ nick: localStorage.getItem('rvw_nickname')||'SPELER2', score:0 }
    });
  } catch(e){ mpStatus('Fout: ' + e.message); }
}
...
function mpStartGame(code, room){
  ...
  document.getElementById('mp-p1-name').textContent = p1nick;
  document.getElementById('mp-p2-name').textContent = p2nick;
  document.getElementById('mp-p1-score').textContent = '€0';
  document.getElementById('mp-p2-score').textContent = '€0';
  document.getElementById('mp-scorebar').classList.add('visible');

  // Spelstaat initialiseren
  initAudio();
  initState(room.rounds || selectedRounds);
  G.mpMode      = true;
  G.mpCode      = code;
  G.mpMyKey     = window.MP.myKey;
  G.mpOppKey    = window.MP.oppKey;
  G.mpRoundStarter = null; // wordt per ronde bepaald via spin-off
  ...
  // Luister naar Firebase acties
  mpUnlisten = window.MPapi.listenRoom(code, mpOnRoomUpdate);

  // Host initialiseert eerste ronde via spin-off
  if(window.MP.role === 'host'){
    setTimeout(() => mpStartSpinOff(), 800);
  } else {
    setBanner('Wachten op spin-off...', 'empty');
    G.locked = true;
  }
}
...
let mpSpinOff = {
  active:    false,
  attempt:   1,       // 1 of 2 (max 2 pogingen voor gelijkspel)
  p1val:     null,
  p2val:     null,
  p1done:    false,
  p2done:    false,
...
function mpStartSpinOff(){
  mpSpinOff = { active:true, attempt:1, p1val:null, p2val:null, p1done:false, p2done:false };

  // Push spin-off start naar Firebase
  window.MPapi.pushAction(G.mpCode, { type:'spinoff_start', attempt:1, ts:Date.now() });

  // Toon modal voor beide spelers
  mpShowSpinOffModal();
}

function mpShowSpinOffModal(){
  const myNick  = localStorage.getItem('rvw_nickname') || 'JIJ';
  const oppNick = mpGetOppNick();
  const attempt = mpSpinOff.attempt;

  const title = attempt === 1 ? '🎡 Spin-off!' : '🎡 Nogmaals!';
  const sub   = 'Wie het hoogste bedrag draait begint de ronde!';

  // Gebruik bestaande showResult modal maar zonder antwoord
  showResult(title, sub, '⚔️', false, null, null);

  // Vervang de OK knop door een draai knop
  const box = document.querySelector('#modal-result .modal-box');
  const existingBtn = box.querySelector('.modal-btn');
  if(existingBtn) existingBtn.remove();

  const spinBtn = document.createElement('button');
  spinBtn.className = 'modal-btn mbtn-ok';
  spinBtn.style.cssText = 'width:100%;margin-top:8px;';
  spinBtn.textContent = '🎡 DRAAI!';
  spinBtn.id = 'spinoff-btn';
  spinBtn.onclick = mpDoSpinOff;
  box.appendChild(spinBtn);

  // Status labels
  const statusDiv = document.createElement('div');
  statusDiv.id = 'spinoff-status';
  statusDiv.style.cssText = 'font-size:12px;color:var(--muted);margin-top:8px;min-height:36px;letter-spacing:1px;text-align:center;';
  statusDiv.innerHTML = '<div id="so-p1-result" style="color:var(--text)"></div><div id="so-p2-result" style="color:var(--text)"></div>';
  box.appendChild(statusDiv);

  document.getElementById('modal-result').classList.add('open');
}

function mpDoSpinOff(){
  const btn = document.getElementById('spinoff-btn');
  if(btn) btn.disabled = true;

  // Draai het wiel — gebruik bestaande doSpin maar intercept het resultaat
  G.locked = false;
  G.phase  = 'spin';
  G.turn   = 'player';

  // Tijdelijk de spinoff handler instellen
  G.mpSpinOffPending = true;
  document.getElementById('modal-result').classList.remove('open');
  doSpin('player');
}

// Wordt aangeroepen vanuit onSpinLand als mpSpinOffPending = true
function mpHandleSpinOffResult(seg){
  G.mpSpinOffPending = false;
  G.locked = true;

  const val = spinOffValue(seg);
  const myKey = window.MP.myKey;

  // Sla mijn resultaat op lokaal
  if(myKey === 'player1') mpSpinOff.p1val = val;
  else mpSpinOff.p2val = val;
  ...
  window.MPapi.pushAction(G.mpCode, {
    ...
    val,
    segLbl:  seg.lbl,
    attempt: mpSpinOff.attempt,
    ts:      Date.now(),
  });

  // Toon eigen resultaat
  mpUpdateSpinOffDisplay();
}

function mpUpdateSpinOffDisplay(){
  const p1val = mpSpinOff.p1val;
  const p2val = mpSpinOff.p2val;
  const p1nick = mpGetP1Nick();
  const p2nick = mpGetP2Nick();

  // Heropen de modal met resultaten
  showResult('🎡 Spin-off!', 'Wachten op tegenstander...', '⚔️', false, null, null);
  const box = document.querySelector('#modal-result .modal-box');
  const existingBtn = box.querySelector('.modal-btn');
  if(existingBtn) existingBtn.remove();

  const statusDiv = document.createElement('div');
  statusDiv.style.cssText = 'font-size:13px;margin-top:8px;text-align:center;';
  statusDiv.innerHTML = `
    <div style="margin-bottom:4px;color:${p1val!==null?'var(--gold)':'var(--muted)'}">
      ${p1nick}: ${p1val !== null ? '€'+p1val.toLocaleString('nl-NL') : '...'}
    </div>
    <div style="color:${p2val!==null?'var(--gold)':'var(--muted)'}">
      ${p2nick}: ${p2val !== null ? '€'+p2val.toLocaleString('nl-NL') : '...'}
    </div>
  `;
  box.appendChild(statusDiv);
  document.getElementById('modal-result').classList.add('open');
...
function mpResolveSpinOff(){
  const p1val = mpSpinOff.p1val;
  const p2val = mpSpinOff.p2val;

  document.getElementById('modal-result').classList.remove('open');

  let starter; // 'player1' of 'player2'

  if(p1val > p2val)       starter = 'player1';
  else if(p2val > p1val)  starter = 'player2';
  else if(mpSpinOff.attempt < 2){
    // Gelijkspel — één keer opnieuw
    mpSpinOff.attempt = 2;
    mpSpinOff.p1val = null;
    mpSpinOff.p2val = null;
    window.MPapi.pushAction(G.mpCode, { type:'spinoff_start', attempt:2, ts:Date.now() });
    setTimeout(() => mpShowSpinOffModal(), 500);
    return;
  } else {
    // Nog steeds gelijk na 2 pogingen → host begint
    starter = 'player1';
  }

  G.mpRoundStarter = starter;
  mpSpinOff.active = false;

  // Push winnaar naar Firebase
  window.MPapi.pushAction(G.mpCode, {
    type:    'spinoff_winner',
    starter,
    ts:      Date.now(),
  ...
  // Start de ronde
  mpBeginRound(starter);
}

function mpBeginRound(starter){
  // Bepaal wie 'player' is (actieve beurt) en wie 'computer' (wacht)
  const iAmStarter = (starter === window.MP.myKey);
  G.turn    = iAmStarter ? 'player' : 'computer';
  G.locked  = !iAmStarter;

  const starterNick = starter === 'player1' ? mpGetP1Nick() : mpGetP2Nick();

  if(window.MP.role === 'host'){
    // Host start de ronde officieel
    startRound();
    // Override de beurt na startRound
    G.turn   = iAmStarter ? 'player' : 'computer';
    G.locked = !iAmStarter;
    mpPushState();
  }
  ...
  if(iAmStarter){
    setBanner('Jij begint! Draai het rad.', 'money');
  } else {
    setBanner(starterNick + ' begint deze ronde...', 'empty');
  }
  updateTurnUI();
}
...
function mpShowDisconnectWarning(){
  setBanner('⚠️ Verbinding tegenstander verbroken... wachten', 'neutral');
}

function mpHandleDisconnect(){
  clearInterval(mpPresenceTimer);
  if(mpUnlisten){ mpUnlisten(); mpUnlisten = null; }
  document.getElementById('mp-scorebar').classList.remove('visible');
...
// ═══════════════════════════════════════════════════════════════════════════
//  FIREBASE SPELSTAAT SYNC
// ═══════════════════════════════════════════════════════════════════════════
function mpPushState(){
  if(!G.mpMode || !G.mpCode) return;
  ...
  const state = {
    ...
    usedLetters: Array.from(G.usedLetters),
    roundScore:  G.roundScore,
    totalScore:  G.totalScore,
    spinVal:     G.spinVal,
    bonusVal:    G.bonusVal,
    onBonus:     G.onBonus,
    activeSegs:  activeSegs.map(s => ({lbl:s.lbl, val:s.val, col:s.col})),
    roundStarter: G.mpRoundStarter,
    ts:          Date.now(),
  };
  window.MPapi.pushGameState(G.mpCode, state);
}

// ── Firebase update ontvangen ────────────────────────────────────────────────
function mpOnRoomUpdate(room){
  if(!room) return;

  // Heartbeat van tegenstander
  if(room.state && room.state.ts){
    // Controleer of het een heartbeat actie is
  }
  if(room.action){
    const a = room.action;
    // Update tegenstander last seen bij elke actie
    if(a.player && a.player !== window.MP.myKey){
      mpOppLastSeen = Date.now();
      mpDisconnectWarned = false;
    }
    mpHandleAction(a);
  ...
  // Gast verwerkt spelstaat van host
  if(window.MP.role === 'guest' && room.state){
    const st = room.state;
    if(!st.ts || st.ts <= mpLastActionTs) return;
    mpLastActionTs = st.ts;
    mpApplyState(st);
  }

  // Scorebar altijd updaten
  mpUpdateScorebar(room);
}

// ── Actie verwerken (beide spelers) ─────────────────────────────────────────
function mpHandleAction(a){
  if(!a || !a.type) return;

  switch(a.type){

    case 'spinoff_start':
      // Gast krijgt het signaal om de spin-off te starten
      if(window.MP.role === 'guest'){
        mpSpinOff.attempt = a.attempt;
        if(a.attempt === 1){
          mpSpinOff = { active:true, attempt:1, p1val:null, p2val:null, p1done:false, p2done:false };
        } else {
          mpSpinOff.p1val = null; mpSpinOff.p2val = null;
        }
        setTimeout(() => mpShowSpinOffModal(), 300);
      }
      break;

    case 'spinoff_result':
      // Tegenstander heeft gedraaid
      if(a.player !== window.MP.myKey){
        if(a.player === 'player1') mpSpinOff.p1val = a.val;
        else mpSpinOff.p2val = a.val;
        mpUpdateSpinOffDisplay();
      }
      // Check of beide klaar zijn
      if(mpSpinOff.p1val !== null && mpSpinOff.p2val !== null){
        // Alleen host beslist
        if(window.MP.role === 'host'){
          setTimeout(() => mpResolveSpinOff(), 1200);
        }
      }
      break;

    case 'spinoff_winner':
      // Gast ontvangt wie begint
      if(window.MP.role === 'guest'){
        setTimeout(() => mpBeginRound(a.starter), 600);
      }
      break;

    case 'heartbeat':
      // Al verwerkt via mpOppLastSeen hierboven
      break;
  }
}

// ── Spelstaat toepassen op gast ──────────────────────────────────────────────
function mpApplyState(st){
  G.round       = st.round;
  G.turn        = st.turn;
  G.phase       = st.phase;
  G.puzzle      = st.puzzle;
  G.revealed    = new Set(st.revealed || []);
  G.usedLetters = new Set(st.usedLetters || []);
  G.roundScore  = st.roundScore  || {player:0, computer:0};
  G.totalScore  = st.totalScore  || {player:0, computer:0};
  G.spinVal     = st.spinVal  || 0;
  G.bonusVal    = st.bonusVal || 0;
  G.onBonus     = st.onBonus  || false;
  G.mpRoundStarter = st.roundStarter || null;
  if(st.activeSegs) activeSegs = st.activeSegs;

  renderPuzzle();
  buildKeyboard();
  updateScores();

  // Bepaal of het mijn beurt is
  const myTurn = (window.MP.myKey === 'player1' && st.turn === 'player') ||
                 (window.MP.myKey === 'player2' && st.turn === 'computer');
  G.locked = !myTurn;

  if(myTurn){
    if(st.phase === 'spin')        setBanner('Jouw beurt! Draai het rad.', 'money');
    else if(st.phase === 'guess')  setBanner('Raad een medeklinker.', 'money');
    else if(st.phase === 'buy_vowel') setBanner('Kies een klinker.', 'money');
  } else {
    setBanner('Tegenstander is aan de beurt...', 'empty');
  }
  updateTurnUI();

  // Ronde pill
  const pill = document.getElementById('round-pill');
  pill.textContent = 'Ronde ' + st.round + ' van ' + G.maxRounds;
}

// ── Scorebar updaten ─────────────────────────────────────────────────────────
function mpUpdateScorebar(room){
  if(!room) return;
  const p1score = room.player1 ? (room.player1.score || 0) : 0;
  const p2score = room.player2 ? (room.player2.score || 0) : 0;
  document.getElementById('mp-p1-score').textContent = '€' + p1score.toLocaleString('nl-NL');
  document.getElementById('mp-p2-score').textContent = '€' + p2score.toLocaleString('nl-NL');

  const myIsP1 = window.MP.myKey === 'player1';
  document.getElementById('mp-p1').classList.toggle('active-mp',
    (G.turn === 'player' && myIsP1) || (G.turn === 'computer' && !myIsP1));
  document.getElementById('mp-p2').classList.toggle('active-mp',
    (G.turn === 'player' && !myIsP1) || (G.turn === 'computer' && myIsP1));
}

// ── Hulpfuncties namen ───────────────────────────────────────────────────────
function mpGetP1Nick(){ return document.getElementById('mp-p1-name').textContent || 'SPELER 1'; }
function mpGetP2Nick(){ return document.getElementById('mp-p2-name').textContent || 'SPELER 2'; }
