...
function updateTurnUI(){
  ...
  const myScore = (G.mpMode && window.MP && window.MP.myKey==='player2') ? G.roundScore.computer : G.roundScore.player;
  ...
  document.getElementById('btn-vowel').disabled=!(isP&&(G.phase==='guess'||(G.phase==='spin'&&G.hasGuessed))&&!G.locked&&myScore>=(G.vowelCost||VOWEL_COST)&&hasUnrevealedVowels()&&!G.suddenDeath);
...
function doSpin(who, forcedRot){
  ...
  if(forcedRot !== undefined){
    ...
    if(G.mpMode && !G.mpSuppressActionPush && mpIsMyTurn()){
      mpPushAction({ type:'spin', totalRot: totalRot, startRot: wheelRot });
    }
  }
...
function handleBuyVowel(){
  ...
  const myScore = (G.mpMode && window.MP && window.MP.myKey==='player2') ? G.roundScore.computer : G.roundScore.player;
  if(myScore<(G.vowelCost||VOWEL_COST)||!hasUnrevealedVowels()) return;
...
function mpProcessOppAction(a){
  ...
  switch(a.type){
    ...
    case 'spin':
      // Tegenstander heeft gedraaid — forceer perfecte synchronisatie
      G.locked = true;
    ...
    case 'round_start':
      ...
      if(window.MP.role === 'guest'){
        ...
        buildKeyboard();
        updateScores();
        drawWheel(wheelRot); // Herteken wiel met correcte activeSegs
        // Pill
        const pill = document.getElementById('round-pill');
        if(a.roundType === 'gold')
