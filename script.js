...
function startRound(){
  ...
  if(G.mpMode && window.MP && window.MP.role==='host'){
    ...
    setTimeout(()=> mpPushAction({ type:'round_start', puzzleIdx, round:G.round, roundType:G.roundType||'normal' }), 50);
  }
  ...
  if(roundType !== 'normal'){
    ...
    setTimeout(()=>{
      ...
      showResult(title, body, icon, false,
        ()=>{ G.locked=false; updateTurnUI(); if(!G.mpMode && G.turn==='computer') setTimeout(computerTurn,1600); },
        theme
      );
    }, 300);
  } else {
...
function doSpin(who, forcedSegIndex){
  G.spinning=true; G.locked=true; updateTurnUI();

  // MP tegenstander replay — kort animatie, dan land op opgegeven segment
  if(forcedSegIndex !== undefined){
    const _segs = activeSegs.length ? activeSegs : SEGS;
    const n = _segs.length;
    // Bereken rotatie die uitkomt op forcedSegIndex
    const segArc = (2*Math.PI) / n;
    const targetAngle = ((forcedSegIndex + 0.5) * segArc);
    const targetRot = wheelRot + (3 * Math.PI * 2) + targetAngle;
    const dur = 2000;
    ...
    function replayFrame(now){
      ...
      drawWheel(startRot + totalRot*ease);
      if(prog < 1){ requestAnimationFrame(replayFrame); return; }
      G.spinning = false;
      wheelRot = startRot + totalRot;
      onLanded(_segs[forcedSegIndex % n], who);
    }
...
function onLanded(seg,who){
  // In MP: als het mijn beurt was, push de actie naar Firebase
  if(G.mpMode && mpIsMyTurn()){
    const _segs = activeSegs.length ? activeSegs : SEGS;
    const segIndex = _segs.indexOf(seg);
    mpPushAction({ type:'spin', segIndex, segVal:seg.val, segLbl:seg.lbl });
  }
  if(seg.val==='BANKRUPT'){
...
function processGuess(letter,who){
  ...
  // In MP: als het mijn beurt was, push de actie
  if(G.mpMode && mpIsMyTurn()){
...
function submitSolve(){
  ...
  if(guess===G.puzzle.a){
    clearTimeout(G.sdInterval);
    if(G.mpMode) mpPushAction({ type:'solve_ok' });
    G.puzzle.a.split('').forEach((_,i)=>{ if(G.puzzle.a[i]!==' ') G.revealed.add(i); });
    renderPuzzle();
    endRound(G.mpMode ? G.turn : 'player');
  } else {
    setBanner('❌ Fout antwoord!','bad');
    if(G.mpMode) mpPushAction({ type:'solve_bad' });
    if(G.suddenDeath){
      setTimeout(()=>{}, 900);
    } else {
...
function endRound(winner){
  ...
  if(G.mpMode) window.MPapi.pushScore(G.mpCode, G.mpMyKey, G.totalScore.player);
  ...
  setTimeout(()=>showResult(title,sub,icon,true,()=>{
    ...
    if(G.mpMode && window.MP.role==='host'){
      ...
      // Gast wacht — host pusht round_start en coinflip via Firebase
      G.locked = true;
...
function mpOnRoomUpdate(room){
  ...
  // Scores bijwerken in scorebar
  const p1s = room.player1 ? (room.player1.score||0) : 0;
  const p2s = room.player2 ? (room.player2.score||0) : 0;
  document.getElementById('mp-p1-score').textContent = '€' + p1s.toLocaleString('nl-NL');
  document.getElementById('mp-p2-score').textContent = '€' + p2s.toLocaleString('nl-NL');

  // Actieve speler highlighten
...
function mpProcessOppAction(a){
  ...
  switch(a.type){
    ...
    case 'spin':
      // Tegenstander heeft gedraaid — speel het wiel na naar hetzelfde segment
      G.locked = true;
      doSpin(oppTurn, a.segIndex);
      break;
    ...
    case 'solve_bad':
      // Tegenstander heeft fout geraden — beurt naar mij
      setBanner('❌ ' + mpGetOppNick() + ' raadde fout!', 'bad');
      setTimeout(() => switchTurn(), 1000);
      break;

    case 'round_start':
      // Host start een nieuwe ronde — gast laadt dezelfde puzzel
      if(window.MP.role === 'guest'){
        // Zoek de puzzel op basis van index
        G.puzzle = PUZZLES[a.puzzleIdx];
        G.revealed = new Set();
        G.usedLetters = new Set();
        G.roundScore = {player:0, computer:0};
        G.bonusVal = 0; G.onBonus = false; G.hasGuessed = false;
        G.round = a.round;
        activeSegs = SEGS.slice();
        // Pas bonus segs toe als het een bonusronde is
        if(a.roundType === 'chrome') mpApplyBonusSegs('chrome');
        else if(a.roundType === 'gold') mpApplyBonusSegs('gold');
        renderPuzzle();
        buildKeyboard();
        updateScores();
        // Pill
        const pill = document.getElementById('round-pill');
        if(a.roundType === 'gold')
          pill.innerHTML = '<span class="star-gold">★</span> Goud Bonus Ronde ' + a.round + ' van ' + G.maxRounds;
        else if(a.roundType === 'chrome')
          pill.innerHTML = '<span class="star-silver">★</span> Zilver Bonus Ronde ' + a.round + ' van ' + G.maxRounds;
        else
          pill.textContent = 'Ronde ' + a.round + ' van ' + G.maxRounds;
      }
      break;
