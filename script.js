...
const DIFF = {
  easy: {
    ...
    solve:      0.80,
    solveChance:0.40,
    failSolve:  0.15,   // kans op Illusionist-fail bij reguliere oplospoging
    thinkDelay: 1400,
  },
  medium: {
    freq:       FREQ_ORDER,
...
function updateTurnUI(){
  ...
  const noConsLeft = !hasUnrevealedConsonants();
  // Klinker kopen mag ook direct (zonder draaien) als er geen medeklinkers meer zijn
  const kanKlinkerKopen = isP && !G.locked && myScore >= vCost && hasUnrevealedVowels() && !G.suddenDeath &&
    (G.phase==='guess' || (G.phase==='spin' && G.hasGuessed) || (G.phase==='spin' && noConsLeft));

  // Draaien mag niet als er geen medeklinkers meer zijn
  document.getElementById('btn-spin').disabled = !(isP && G.phase==='spin' && !G.locked && !G.suddenDeath && !noConsLeft);
...
function handleSpin(){
  ...
  if(!hasUnrevealedConsonants()) return; // Niet draaien als er geen medeklinkers meer zijn
...
function handleBuyVowel(){
  if(G.mpMode && !mpIsMyTurn()) return;
  if(!G.mpMode && G.turn!=='player') return;
  // Klinker kopen mag ook direct als er geen medeklinkers meer zijn
  const _noConsLeft = !hasUnrevealedConsonants();
  if((G.phase!=='guess'&&(G.phase!=='spin'||(!G.hasGuessed&&!_noConsLeft)))||G.locked) return;
  ...
  setTimeout(()=>{
    ...
    if(hit){
      ...
      if(isVowel){
        ...
        // Banner: rad gesloten als geen medeklinkers meer over
        if(!hasUnrevealedConsonants()){
          setTimeout(()=>setBanner('🔒 Alleen klinkers over! Rad is gesloten.','neutral'), 800);
        }
        if(!G.mpMode && who==='computer') setTimeout(()=>computerTurn(),1300);
      }
    } else {
...
function computerTurn(){
  ...
  showThinking(true);
  const cfg = Object.assign({}, DIFF[G.difficulty]);
  // TEST_MODE: force Illusionist to always trigger
  if(window.TEST_MODE){ cfg.solveChance = 1.0; cfg.failSolve = 1.0; cfg.solve = 0.0; }
  const total = G.puzzle.a.replace(/ /g,'').length;
  const pct   = G.revealed.size / Math.max(1, total);
  const missingPositions = total - G.revealed.size;
  const noConsonantsLeft = !hasUnrevealedConsonants();
  const hasVowels        = hasUnrevealedVowels();
  const canBuyVowel      = G.roundScore.computer >= (G.vowelCost || VOWEL_COST);

  setTimeout(()=>{
    showThinking(false);

    // ── 1. Exact 1 positie ontbreekt — zeker oplossen ────────────────────────
    if(missingPositions <= 1){
      setBanner('🤖 Oplossen…','money');
      setTimeout(()=>endRound('computer'), 1400); return;
    }

    // ── 2. Klinker-valstrik fix — geen medeklinkers meer ─────────────────────
    if(noConsonantsLeft){
      // Niet draaien — direct klinker kopen of geforceerd oplossen
      // Klinker kopen als mogelijk
      if(hasVowels && canBuyVowel){
      ...
      // Kan geen klinker kopen — geforceerd oplossen
      // Ligt zichtbaarheid onder drempel? Dan Illusionist-fail
      if(pct < cfg.solve){
        // Illusionist: 2-stap TV-show spanning
        showResult('Computer lost op...', 'De computer probeert het woord te raden!', '🤖', false, null);
        setTimeout(()=>{
          document.getElementById('modal-result').classList.remove('open');
          setTimeout(()=>{
            showResult('Computer Faalt', 'De computer heeft het niet goed geraden. U bent weer aan de beurt!', '🤖', false, ()=>switchTurn('computer'));
          }, 250);
        }, 2000); return;
      }
      // Boven drempel — gewoon oplossen
      setBanner('🤖 Oplossen…','money');
      setTimeout(()=>endRound('computer'), 1400); return;
    }

    // ── 3. Reguliere oplospoging met drempel + Illusionist ───────────────────
    if(pct >= cfg.solve && Math.random() < cfg.solveChance){
      // Veiligheidsklep: >= 90% zichtbaar = nooit falen
      const failChance = pct >= 0.90 ? 0 : (cfg.failSolve || 0);
    ...
    // ── 4. Klinker kopen? Alleen na draaien ──────────────────────────────────
    if(G.hasGuessed && canBuyVowel && hasVowels && pct > cfg.vowelMin && Math.random() < cfg.vowel){
      const av = [...VOWELS].filter(v=>!G.usedLetters.has(v) && G.puzzle.a.includes(v));
      if(av.length > 0){
        G.phase='buy_vowel'; G.spinVal=0;
        const v = G.difficulty==='hard' || G.difficulty==='expert'
    ...
    // ── 5. Draaien — alleen als er nog medeklinkers zijn ────────────────────
    if(noConsonantsLeft){
      // Geen medeklinkers meer maar ook geen andere optie — geforceerd oplossen
      // (dit is een vangnet: stap 2 had dit al moeten afhandelen)
      setBanner('🤖 Oplossen…','money');
      setTimeout(()=>endRound('computer'), 1400); return;
    }
    doSpin('computer');
...
async function endGame(){
  ...
  setTimeout(async ()=>{
    ...
    setTimeout(async ()=>{
      ...
      if(outcome === 'win'){
        // Check of score in top 10 past — arcade highscore flow
        await arcadeHighscoreFlow(myTotal, G.difficulty);
      }
      // Altijd leaderboard tonen voor credits
...
function handlePlayButton(){
  ...
  // Naam is optioneel voor single player, verplicht voor multiplayer
  ...
  if(val.length >= 2){
    ...
    error.textContent = 'Minimaal 2 tekens vereist';
    input.focus(); return;
  }
  // Naam leeg of te kort? Geen probleem voor single player — speel als anoniem
...
window.addEventListener('load', ()=>{
  ...
  // Pre-fill naam — direct uit localStorage (werkt ook als FB nog niet geladen is)
  const _savedNick = localStorage.getItem('rvw_nickname');
  const _nickInput = document.getElementById('nickname-input');
  if(_savedNick && _nickInput) _nickInput.value = _savedNick;
  // Ook via FB na korte wachttijd (als fallback)
  setTimeout(()=>{
    if(window.FB){
...
// ── Arcade Highscore Flow ────────────────────────────────────────────────────
async function arcadeHighscoreFlow(score, difficulty){
  if(!window.FB || !window.FB.checkTopN) return;
  try {
    const qualifies = await window.FB.checkTopN(difficulty, score);
    if(!qualifies) return;
    // Score haalt top 10 — toon naam invoer modal
    const nick = window.FB.getNickname() || '';
    const sub = document.getElementById('arcade-hs-sub');
    if(sub) sub.textContent = 'Gefeliciteerd! Je staat in de Top 10!';
    const inp = document.getElementById('arcade-hs-input');
    if(inp) inp.value = nick;
    // Sla score en difficulty op voor gebruik bij opslaan
    window._arcadeScore = score;
    window._arcadeDiff  = difficulty;
    return new Promise(resolve => {
      window._arcadeResolve = resolve;
      document.getElementById('modal-arcade-highscore').classList.add('open');
    });
  } catch(e){ console.error('Arcade highscore error:', e); }
}

async function arcadeHsSave(){
  const inp = document.getElementById('arcade-hs-input');
  const err = document.getElementById('arcade-hs-error');
  const name = inp ? inp.value.trim().toUpperCase() : '';
  if(name.length < 2){
    if(err) err.textContent = 'Minimaal 2 tekens vereist';
    return;
  }
  if(!/^[A-Z0-9 _\-\.]+$/.test(name)){
    if(err) err.textContent = 'Alleen letters, cijfers en . _ - toegestaan';
    return;
  }
  // Knop disablen om dubbelklik te voorkomen
  const btn = document.querySelector('#modal-arcade-highscore .mbtn-ok');
  if(btn) { btn.disabled = true; btn.textContent = 'OPSLAAN...'; }

  if(window.FB) window.FB.setNickname(name);

  // WACHT op Firebase voordat de modal sluit
  if(window.FB && window.FB.saveScore){
    await window.FB.saveScore(window._arcadeDiff, name, window._arcadeScore);
  }

  document.getElementById('modal-arcade-highscore').classList.remove('open');
  if(window._arcadeResolve) { window._arcadeResolve(); window._arcadeResolve = null; }

  if(btn) { btn.disabled = false; btn.textContent = '💾 OPSLAAN'; }
}

function arcadeHsSkip(){
  document.getElementById('modal-arcade-highscore').classList.remove('open');
  if(window._arcadeResolve) { window._arcadeResolve(); window._arcadeResolve = null; }
}
